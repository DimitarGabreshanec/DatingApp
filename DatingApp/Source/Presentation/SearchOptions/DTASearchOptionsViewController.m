...
@property (nonatomic, strong) NSDictionary *countryCodesByNames;
...
- (void)viewDidLoad {
    ...
    self.advancedSearchOptionsButton.layer.cornerRadius = 10.0f;
    ...
    self.countryCodesByNames = [self countryCodesByName];
    ...
    [DTAAPI fetchStaticResourceWithKey:DTAAPIStaticResourcesTypeEducations completion:^(NSError *error, NSArray *result) {
         if (!error) {
             ...
             NSMutableArray *tmpArray = [[NSMutableArray alloc] initWithArray:result];
             NSMutableDictionary *tmpDic = [[NSMutableDictionary alloc] initWithDictionary:tmpArray[3]];
             [tmpArray removeObjectAtIndex:3];
             [tmpArray insertObject:tmpDic atIndex:0];
             
             NSMutableArray *educations = [NSMutableArray arrayWithArray:tmpArray];
    ...
    [DTAAPI fetchStaticResourceWithKey:DTAAPIStaticResourcesTypeOrientation completion:^(NSError *error, NSArray *result) {
        if (!error) {
            ...
            NSMutableArray *tmpArray = [[NSMutableArray alloc] initWithArray:result];
            NSMutableDictionary *tmpDic = [[NSMutableDictionary alloc] initWithDictionary:tmpArray[2]];
            [tmpArray removeObjectAtIndex:2];
            [tmpArray addObject:tmpDic];
            
            
            NSMutableArray *orientationArr = [NSMutableArray arrayWithArray:tmpArray];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    if (indexPath.section ==  DTASearchOptionsTableSectionTypeBasicOptions) {
        if (indexPath.row ==  DTABasicSearchOptionsCellTypeDistanceLimit) {
            ...
            cell.distanceSlider.upperValue = [DTASearchOptionsManager sharedManager].searchOptions.nearbyRadius.floatValue;
            
            if (cell.distanceSlider.upperValue <= 300) {
                  cell.selectedDistaceLabel.text = [NSString stringWithFormat:@"%i miles", (int) cell.distanceSlider.upperValue];
            }
            else {
            ...
            int minimalAge = (int) cell.agesIntervalSlider.lowerValue;
            int maximalAge = (int) cell.agesIntervalSlider.upperValue;
            cell.selectedAgeRangeLabel.text = [NSString stringWithFormat:@"%i-%i", minimalAge, maximalAge];
            
            CGPoint lowerCenter;
            lowerCenter.x = (cell.agesIntervalSlider.lowerCenter.x + cell.agesIntervalSlider.frame.origin.x);
            lowerCenter.y = (cell.agesIntervalSlider.center.y - 30.0f);
            cell.lowerLabel.center = lowerCenter;
            [cell.lowerLabel setTitle:[NSString stringWithFormat:@"%d", (int)cell.agesIntervalSlider.lowerValue] forState:UIControlStateNormal];
            
            
            CGPoint upperCenter;
            upperCenter.x = (cell.agesIntervalSlider.upperCenter.x + cell.agesIntervalSlider.frame.origin.x);
            upperCenter.y = (cell.agesIntervalSlider.center.y - 30.0f);
            cell.upperLabel.center = upperCenter;
            [cell.upperLabel setTitle:[NSString stringWithFormat:@"%d", (int)cell.agesIntervalSlider.upperValue] forState:UIControlStateNormal];
        ...
        if (indexPath.row == DTAAdditionalInfoCellTypeLocation) {
            ...
            [cell.imageOption setImage:[[UIImage imageNamed:@"global"] imageWithRenderingMode:UIImageRenderingModeAlwaysOriginal]];
        }
        else if (indexPath.row == DTAAdditionalInfoCellTypeEthnicGroup) {
            [cell.imageOption setImage:[[UIImage imageNamed:@"ethnicity"] imageWithRenderingMode:UIImageRenderingModeAlwaysOriginal]];
        }
        else if (indexPath.row == DTAAdditionalInfoCellTypeRelationship) {
            [cell.imageOption setImage:[[UIImage imageNamed:@"relation"] imageWithRenderingMode:UIImageRenderingModeAlwaysOriginal]];
        }
        else if (indexPath.row == DTAAdditionalInfoCellTypeProfession) {
            [cell.imageOption setImage:[[UIImage imageNamed:@"brefcase"] imageWithRenderingMode:UIImageRenderingModeAlwaysOriginal]];
        }
        else if (indexPath.row == DTAAdditionalInfoCellTypeEducation) {
            [cell.imageOption setImage:[[UIImage imageNamed:@"education"] imageWithRenderingMode:UIImageRenderingModeAlwaysOriginal]];
        }
        else if (indexPath.row == DTAAdditionalInfoCellTypeCountryOfOrigin) {
            [cell.imageOption setImage:[[UIImage imageNamed:@"global"] imageWithRenderingMode:UIImageRenderingModeAlwaysOriginal]];
        }
        else if (indexPath.row == DTAAdditionalInfoCellTypeReligion) {
            [cell.imageOption setImage:[[UIImage imageNamed:@"book"] imageWithRenderingMode:UIImageRenderingModeAlwaysOriginal]];
        }
        else if (indexPath.row == DTAAdditionalInfoCellTypeGoals) {
            [cell.imageOption setImage:[[UIImage imageNamed:@"relation"] imageWithRenderingMode:UIImageRenderingModeAlwaysOriginal]];
        }
        else if (indexPath.row == DTAAdditionalInfoCellTypeWantKids) {
            [cell.imageOption setImage:[[UIImage imageNamed:@"children"] imageWithRenderingMode:UIImageRenderingModeAlwaysOriginal]];
        }
        else if (indexPath.row == DTAAdditionalInfoCellTypeHaveKids) {
            [cell.imageOption setImage:[[UIImage imageNamed:@"kid"] imageWithRenderingMode:UIImageRenderingModeAlwaysOriginal]];
        }
        else {
            [cell.imageOption setImage:[[UIImage imageNamed:@"global"] imageWithRenderingMode:UIImageRenderingModeAlwaysOriginal]];
        }
...
- (UIView *)pickerView:(UIPickerView *)pickerView viewForRow:(NSInteger)row forComponent:(NSInteger)component reusingView:(UIView *)view {
    ...
    if (self.pickerState == DTAAdditionalInfoCellTypeCountryOfOrigin) {
        if (!view)
        {
            CGFloat width = pickerView.frame.size.width;
            view = [[UIView alloc] initWithFrame:CGRectMake(0, 0, width, 34)];
            
            UIImageView *flagView = [[UIImageView alloc] initWithFrame:CGRectMake(60, 2, 30, 30)];
            flagView.contentMode = UIViewContentModeScaleAspectFit;
            flagView.tag = 2;
            [view addSubview:flagView];
            
            UILabel *label = [[UILabel alloc] initWithFrame:CGRectMake(95, 2, width - 75, 30)];
            label.backgroundColor = [UIColor clearColor];
            label.tag = 1;
            label.font = [UIFont fontWithName:@"PTSans-Bold" size:24];
            label.textColor = colorCreamCan;
            [view addSubview:label];
        }
        
        NSMutableString *countryName = [[NSMutableString alloc] init];
        if ([[DTASearchOptionsManager sharedManager].arrayOfCountries[row] isKindOfClass:[NSDictionary class]]) {
            NSDictionary *dict = [DTASearchOptionsManager sharedManager].arrayOfCountries[row];
            countryName= dict[@"title"];
        } else {
            countryName = [DTASearchOptionsManager sharedManager].arrayOfCountries[row];
        }
        
        NSMutableString *countryCode = [[NSMutableString alloc] initWithString:@""];
        if ([self.countryCodesByNames objectForKey:countryName]) {
            countryCode = [self.countryCodesByNames objectForKey:countryName];
        } else {
            NSLog(@"Country name is %@", countryName);
            if ([countryName isEqualToString:@"Antigua and Barbuda"]) {
                countryCode = [self.countryCodesByNames objectForKey:@"Antigua & Barbuda"];
            } else if ( [countryName isEqualToString:@"Bosnia and Herzegovina"] ){
                countryCode = [self.countryCodesByNames objectForKey:@"Bosnia & Herzegovina"];
            } else if ( [countryName isEqualToString:@"China"] ){
                countryCode = [self.countryCodesByNames objectForKey:@"China mainland"];
            } else if ( [countryName isEqualToString:@"Congo, Democratic Republic of the"] ){
                countryCode = [self.countryCodesByNames objectForKey:@"Congo - Kinshasa"];
            } else if ( [countryName isEqualToString:@"Congo, Republic of the"] ){
                countryCode = [self.countryCodesByNames objectForKey:@"Congo - Brazzaville"];
            } else if ( [countryName isEqualToString:@"Cote d'Ivoire"] ){
                countryCode = [self.countryCodesByNames objectForKey:@"Côte d’Ivoire"];
            } else if ( [countryName isEqualToString:@"Czech Republic"] ){
                countryCode = [self.countryCodesByNames objectForKey:@"Czechia"];
            } else if ( [countryName isEqualToString:@"East Timor (Timor-Leste)"] ){
                countryCode = [self.countryCodesByNames objectForKey:@"Timor-Leste"];
            } else if ( [countryName isEqualToString:@"Macedonia"] ){
                countryCode = [self.countryCodesByNames objectForKey:@"North Macedonia"];
            } else if ( [countryName isEqualToString:@"Micronesia, Federated States of"] ){
                countryCode = [self.countryCodesByNames objectForKey:@"Micronesia"];
            } else if ( [countryName isEqualToString:@"Myanmar (Burma)"] ){
                countryCode = [self.countryCodesByNames objectForKey:@"Myanmar [Burma]"];
            } else if ( [countryName isEqualToString:@"Saint Kitts and Nevis"] ){
                countryCode = [self.countryCodesByNames objectForKey:@"St Kitts & Nevis"];
            } else if ( [countryName isEqualToString:@"Saint Lucia"] ){
                countryCode = [self.countryCodesByNames objectForKey:@"St Lucia"];
            } else if ( [countryName isEqualToString:@"Saint Vincent and the Grenadines"] ){
                countryCode = [self.countryCodesByNames objectForKey:@"St Vincent & Grenadines"];
            } else if ( [countryName isEqualToString:@"Sao Tome and Principe"] ){
                countryCode = [self.countryCodesByNames objectForKey:@"São Tomé & Príncipe"];
            } else if ( [countryName isEqualToString:@"Swaziland"] ){
                countryCode = [self.countryCodesByNames objectForKey:@"Eswatini"];
            } else if ( [countryName isEqualToString:@"The Bahamas"] ){
                countryCode = [self.countryCodesByNames objectForKey:@"Bahamas"];
            } else if ( [countryName isEqualToString:@"The Gambia"] ){
                countryCode = [self.countryCodesByNames objectForKey:@"Gambia"];
            } else if ( [countryName isEqualToString:@"Trinidad and Tobago"] ){
                countryCode = [self.countryCodesByNames objectForKey:@"Trinidad & Tobago"];
            } else if ( [countryName isEqualToString:@"United States of America"] ){
                countryCode = [self.countryCodesByNames objectForKey:@"United States"];
            } else if ( [countryName isEqualToString:@"Vatican City (Holy See)"] ){
                countryCode = [self.countryCodesByNames objectForKey:@"Vatican City"];
            }            
        }
        ...
        {
            image = [UIImage imageNamed:imagePath inBundle:[NSBundle bundleForClass:[DTAEditProfileViewController class]] compatibleWithTraitCollection:nil];
        } else  {
