...
@interface DTARegisterViewController () <UITextFieldDelegate, InstagramViewControllerDelegate,ASAuthorizationControllerDelegate, ASAuthorizationControllerPresentationContextProviding>
...
- (IBAction)actionTapFacebook:(id)sender {
    ...
    if (!IS_HOST_REACHABLE) {
        ...
        if ([[FBSDKAccessToken currentAccessToken] hasGranted:@"email"] && [[FBSDKAccessToken currentAccessToken] hasGranted:@"public_profile"]){
            [weakSelf loginViaFacebookWithResult:nil];
        } else {
...
- (void)authorizationController:(ASAuthorizationController *)controller didCompleteWithError:(NSError *)error  API_AVAILABLE(ios(13.0)) {
    ...
    switch (error.code) {
        case ASAuthorizationErrorCanceled:
            errorMsg = @"AppleSignAuthorizationErrorCanceled";
            break;
        case ASAuthorizationErrorFailed:
            errorMsg = @"AppleSignAuthorizationErrorFailed";
            break;
        case ASAuthorizationErrorInvalidResponse:
            errorMsg = @"AppleSignAuthorizationErrorInvalidResponse";
            break;
        case ASAuthorizationErrorNotHandled:
            errorMsg = @"AppleSignAuthorizationErrorNotHandled";
            break;
        case ASAuthorizationErrorUnknown:
            errorMsg = @"AppleSignAuthorizationErrorUnknown";
            break;
    }
        
    [self showAlertController:@"Error" andMessage:errorMsg];
}
...
- (void)loginViaFacebookWithResult:(FBSDKLoginManagerLoginResult *)result {
    ...
    NSString *tmpToken;
    if (result == nil){
        tmpToken = [FBSDKAccessToken currentAccessToken].tokenString;
    } else {
        tmpToken= result.token.tokenString;
    }
    
    self.token = tmpToken;
    __weak typeof (self) weakSelf = self;
    
    [DTAAPI loginViaFacebookToken:tmpToken completion:^(NSError *error, NSArray *result) {
        ...
        if (!error) {
            ...
            NSMutableDictionary* parameters = [NSMutableDictionary dictionary];
                [parameters setValue:@"id, email, first_name, last_name,picture.type(large), gender,birthday,interested_in" forKey:@"fields"];
            ...
            [[[FBSDKGraphRequest alloc] initWithGraphPath:@"me" parameters:parameters]
                ...
                NSString *profile = result[@"id"];
                
                [APP_DELEGATE.hud dismiss];
            }];
            
            [self showAlertController:@"Error" andMessage:@"Grant Facebook Profile failed"];
            [APP_DELEGATE.hud dismiss];
        } else if (error.code == 401) {
