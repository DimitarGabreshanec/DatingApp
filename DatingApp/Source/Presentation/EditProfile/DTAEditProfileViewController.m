...
//            NSLog(@"arrayOfProfession = %@", weakSelf.arrayOfProfession);
    ...
    [DTAAPI fetchStaticResourceWithKey : DTAAPIStaticResourcesTypeEducations completion:^(NSError *error, NSArray *result) {
        ...
        if (!error) {
            ...
            weakSelf.arrayOfEducation = tmpArray;
            [weakSelf.pickerView reloadAllComponents];
...
//            NSLog(@"arrayOfReligion = %@", weakSelf.arrayOfReligion);
    ...
    [DTAAPI fetchStaticResourceWithKey : DTAAPIStaticResourcesTypeOrientation completion:^(NSError *error, NSArray *result) {
        ...
        if (!error) {
            ...
            weakSelf.arrayOfOrientation = tmpArray;
            [weakSelf.pickerView reloadAllComponents];
        ...
        case DTAProfileSectionTypeMiddle: {
             if(indexPath.row == DTAProfileCellTypeFavoriteThings) {
                ...
                if ([self.editingUser.favoriteJoll isEqualToString:@""] || self.editingUser.favoriteJoll == nil)
                    [cell configureCellWithSummary:@"Enter your favorite African dish   " sender:self row:indexPath.row];
                else
                    [cell configureCellWithSummary:self.editingUser.favoriteJoll sender:self row:indexPath.row];
                cell.labelQuestion.text = @"What is your favorite African dish?";
                return cell;
            }
                ...
                if ([self.editingUser.dreamParents isEqualToString:@""] || self.editingUser.dreamParents == nil)
                    [cell configureCellWithSummary:@"Enter your favorite African city   " sender:self row:indexPath.row];
                else
                [cell configureCellWithSummary:self.editingUser.dreamParents sender:self row:indexPath.row];
                cell.labelQuestion.text = @"What is your favorite African city?";
                return cell;
            }
...
//    if(!error && (!self.editingUser.avatar.length && !self.avatarImage)) {
    ...
    //DEV
        ...
        [self showAlertWithTitle:@"Error" andMessage:@"Please select for want children"];
        error = YES;
    }
...
- (UIView *)pickerView:(UIPickerView *)pickerView viewForRow:(NSInteger)row forComponent:(NSInteger)component reusingView:(UIView *)view {
    ...
    if (self.pickerState == DTAProfileCellTypeCountryOrigin) {
        if (!view)
        {
            ...
            label.tag = 1;
            label.font = [UIFont fontWithName:@"PTSans-Bold" size:26];
            label.textColor = colorCreamCan;
            [view addSubview:label];
        }
                        
        NSDictionary *dict = self.arrayOfCountryOrigin[row];
        NSString *countryName = [[NSString alloc] initWithString:dict[@"title"]];
...
- (void)textViewDidBeginEditing:(UITextView *)textView {
    ...
    if(textView.tag == DTAProfileCellTypeSummary){
        ...
        if ([textView.text isEqualToString:@"Enter your favorite African dish   "]) {
            textView.text = @"";
        }
    }
        ...
        if ([textView.text isEqualToString:@"Enter your favorite African city   "]) {
            textView.text = @"";
        }
    }
}
- (void)textViewDidEndEditing:(UITextView *)textView {
    ...
    if(textView.tag == DTAProfileCellTypeSummary){
        ...
        if ([textView.text isEqualToString:@""]) {
            textView.text = @"Enter your favorite African dish   ";
        }
    }
        ...
        if ([textView.text isEqualToString:@""]) {
            textView.text = @"Enter your favorite African city   ";
        }
    }
...
//        }
        ...
        if ([newStr isEqualToString:@""]) {
            textView.text = @"Enter your favorite African dish   ";
        } else if ([newStr containsString:@"Enter your favorite African dish"]){
            textView.text = @"";
        }
        ...
        if ([newStr isEqualToString:@""]) {
            textView.text = @"Enter your favorite African city   ";
        } else if ([newStr containsString:@"your favorite African city"]){
            textView.text = @"";
        }
